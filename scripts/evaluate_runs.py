...
def make_env(env_name, config_name, model_path=None):
    ...
    if "BASELINE" in config_name or "VARIABLE_KP" in config_name:
        ...
        use_fixed = False
    elif "SPD_ONLY" in config_name or "_SPD_" in config_name:
        arm_config["impedance_mode"] = "riemannian_kp"
        use_spd = True
    elif "LIE_ONLY" in config_name or "_LIE_" in config_name:
        arm_config["impedance_mode"] = "variable_kp"
        use_lie = True
    elif "DIAG" in config_name:
        arm_config["impedance_mode"] = "variable_kp"
        use_diag = True
    elif "FULL_GRL" in config_name or "_FULLGRL_" in config_name:
        arm_config["impedance_mode"] = "riemannian_kp"
        use_spd = True
        use_lie = True
    else:
...
def parse_run_name(run_name):
    ...
    # Extract gamma
    g_match = re.search(r"_G(\d+\.\d+)_", run_name) or re.search(r"GAMMA(\d+\.\d+)", run_name, re.IGNORECASE)
    gamma = float(g_match.group(1)) if g_match else 0.95
    
    # The config name is the run name without _SEED_X
    config = re.sub(r"_SEED_\d+", "", run_name)
    ...
    if env_str == "DOOR":
        env_real = "Door"
    elif env_str == "TILTEDWIPE" or env_str == "WIPE":
        env_real = "TiltedWipe"
...
def main():
    ...
    parser.add_argument("--episodes", type=int, default=100, help="Number of episodes to evaluate per seed (default: 100)")
    parser.add_argument("--env", type=str, default="all", help="Environment to evaluate (e.g., TiltedWipe)")
    parser.add_argument("--runs", type=str, nargs="+", default=[], help="Specific run names to evaluate (without _SEED_X suffix)")
    ...
    for entry in os.listdir(log_dir):
        if os.path.isdir(os.path.join(log_dir, entry)):
            ...
            if parsed:
                ...
                # Filter by exact run name if --runs is provided
                if args.runs and config not in args.runs:
                    continue
                    
                model_path = os.path.join(log_dir, entry, "best_model.zip")
                if os.path.exists(model_path):
                    found_runs.add(config)
                    models_found.append({
                        "run_name": entry,
                        "env": env_real,
                        "config": config,
    ...
    # Assert missing runs
    if args.runs:
        missing_runs = [r for r in args.runs if r not in found_runs]
        if missing_runs:
            print("\n" + "!" * 50)
            print("WARNING: The following requested runs were NOT found in logs/best_models:")
            for m in missing_runs:
                print(f" - {m}")
            print("!" * 50 + "\n")
            
            if len(missing_runs) == len(args.runs):
                print("None of the requested runs were found. Exiting.")
                return
    
    df = pd.DataFrame(models_found)
    final_results = []
    
    env_list = ["NutAssemblySquare", "Door", "TiltedWipe"]
    if args.env != "all":
        env_list = [args.env]
    ...
    for env in env_list:
        ...
        configs_to_eval = df_env["config"].unique()
        ...
        for config in configs_to_eval:
            ...
            print(f"\nEvaluating {env} | {config} | Gamma: {latest_gamma} | Seeds: {df_target['seed'].tolist()}")
            ...
            for _, row in df_target.iterrows():
                ...
                try:
                    num_ep = 1 if args.test else args.episodes
            ...
            if config_metrics:
                # Find best seed
                if env == "TiltedWipe" and "wipe_percentage" in config_metrics[0]:
                    best_res = max(config_metrics, key=lambda x: x["wipe_percentage"])
                    best_metric = f"{best_res['wipe_percentage']*100:.1f}% Wipe"
                else:
                    best_res = max(config_metrics, key=lambda x: x["success"])
                    best_metric = f"{best_res['success']*100:.1f}% Success"
                    
                print(f"  => Best Seed: {best_res['seed']} ({best_metric})")
                
                agg = { "env": env, "config": config, "gamma": latest_gamma, "seeds_count": len(config_metrics), "best_seed": best_res['seed'] }
    ...
    res_df = new_res_df
    res_df.to_csv(os.path.join(output_dir, "evaluation_metrics.csv"), index=False)
    
    print(f"\nSaved metrics to {output_dir}/evaluation_metrics.csv")

    # LaTeX generation logic expects config_list
    config_list = res_df["config"].unique()

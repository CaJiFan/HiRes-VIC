...
class GeometricWrapper(gym.Wrapper):
    ...
    def step(self, rl_action):
        ...
        if not self.use_spd_manifold and self.llm_planner is not None:
            # Leaky-Cube Bi-Linear Residual Mapping
            # Squashes initial noise to anchor to LLM prior, but preserves linear gradients (alpha=0.2)
            a_rl_raw = action[:self.prior_dim]
            a_rl = 0.2 * a_rl_raw + 0.8 * (a_rl_raw ** 3)
        ...
        if self.use_spd_manifold:
            ...
            def get_spd_matrix(params, is_residual=False):
                m_params = params.copy()
                if is_residual:
                    # Leaky-Cube: Squash initial noise to prevent 4.5x physics explosions!
                    m_params[0:3] = 0.2 * m_params[0:3] + 0.8 * (m_params[0:3] ** 3)
                    m_params[3:6] = 0.2 * m_params[3:6] + 0.8 * (m_params[3:6] ** 3)
                    
                    # Apply full scale. Because of Leaky Cube, initial noise only scales by ~1.3x, but max reach is 4.5x!
                    m_params[0:3] = m_params[0:3] * 1.5  # Max scale ~ e^1.5 = 4.4x multiplier
            ...
            # If we are using LLM, K_rl is a residual perturbation. Otherwise, it's an absolute matrix.
            K_rl = get_spd_matrix(action[:6], is_residual=(self.llm_planner is not None))
            ...
            if self.llm_planner is not None:
                # K_llm is parsed as an absolute matrix
                K_llm = get_spd_matrix(self.current_prior[:6], is_residual=False)
                K_llm = K_llm + _EPS * np.eye(3)
                
                # Riemannian Residual Policy Learning via Canonical Group Action
                # We project the RL agent's output as a geometric perturbation around the LLM prior
                K_llm_sqrt = spla.fractional_matrix_power(K_llm, 0.5)
                kp_matrix = np.real(K_llm_sqrt @ K_rl @ K_llm_sqrt)

                # Leaky-Cube + Bi-linear residual for remaining Euclidean prior dims (like orientation kp_rot)
                if self.prior_dim > 6:
                    a_rl_raw = action[6:self.prior_dim]
                    a_rl = 0.2 * a_rl_raw + 0.8 * (a_rl_raw ** 3)
                    a_prior = self.current_prior[6:self.prior_dim]
                    action[6:self.prior_dim] = np.where(a_rl > 0, 
                                                        a_prior + a_rl * (1.0 - a_prior), 
                                                        a_prior + a_rl * (a_prior + 1.0))
            else:
            ...
            # kp_trans[:3] — log-space decoding (matches _kp_trans_to_norm_log in planner
            #                 and the manifold's own log-scale parameterisation)
            log_kp_trans = log_min + 0.5 * (kp_raw[:3] + 1.0) * (log_max - log_min)
            kp_trans_scaled = np.exp(log_kp_trans)  # 3-dim physical kp_trans

            # kp_rot[3:6] — linear-space decoding (matches _kp_rot_to_norm in planner).
            # Previously decoded through log-space, which collapsed values like
            # kp_rot=20 → ~1.4 N·m/rad (near-zero) and flatlined DIAG performance.
